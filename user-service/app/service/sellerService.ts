...
export class SellerService {
	...
	async JoinSellerProgram(event: APIGatewayProxyEventV2) {
		try {
			...
			const input = plainToClass(SellerProgramInput, JSON.parse(event.body!));
			const error = await AppValidationError(input);
			if (error) {
				return ErrorResponse(404, error);
			}
			const { firstName, lastName, phone, address } = input;
	...
	async EditPaymentMethods(event: APIGatewayProxyEventV2) {
		try {
			...
			const input = plainToClass(PaymentMethodInput, JSON.parse(event.body!));
			const error = await AppValidationError(input);
			if (error) {
				return ErrorResponse(404, error);
			}
